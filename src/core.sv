...
// COMPUTE CORE (RV32IM + INT4 + Sparsity + ROB + L1 Cache + 128-bit Vector + RVV 1.0)
...
module core #(
    ...
    // ---- Per-thread vector signals (Phase 7 & RVV 1.0) ----
    ...
    // ---- Decoded vector control signals (Phase 7 & RVV 1.0) ----
    ...
    // Active thread mask from scheduler
    wire [THREADS_PER_BLOCK-1:0] thread_active_mask;
    ...
    scheduler #(
        ...
        .current_pc(current_pc),
        .next_pc(next_pc),
        .thread_active_mask(thread_active_mask),
        .rs1_zero(rs1_zero),
        .rs2_zero(rs2_zero),
    ...
    generate
        for (i = 0; i < THREADS_PER_BLOCK; i = i + 1) begin : threads
            wire thread_en = (i < thread_count) && thread_active_mask[i];
            ...
            l1_cache #(
                ...
                .clk(clk), .reset(reset),
                .core_read_valid(lsu_raw_rv),
                .core_read_address(lsu_raw_ra),
                .core_read_ready(lsu_cache_rr),
                .core_read_data(lsu_cache_rd),
                .core_write_valid(lsu_raw_wv),
                .core_write_address(lsu_raw_wa),
                .core_write_data(lsu_raw_wd),
                .core_write_ready(lsu_cache_wr),
            ...
            registers registers_instance (
                ...
                .block_id(block_id),
                .thread_id(i[7:0]),
                .thread_count({{(8-$clog2(THREADS_PER_BLOCK)-1){1'b0}}, thread_count}),
                .decoded_reg_write_enable(decoded_reg_write_enable),
                .decoded_rd_address(decoded_rd_address),
                .decoded_rs1_address(decoded_rs1_address),
                .decoded_rs2_address(decoded_rs2_address),
            ...
            // ------------------------------------------------------------------
            // Vector Registers (Phase 7 & RVV 1.0)
            vreg vreg_instance (
                .clk(clk), .reset(reset),
                .enable(thread_en),
                .core_state(core_state),
            ...
            // ------------------------------------------------------------------
            // Vector ALU (Phase 7 & RVV 1.0)
            vec_alu vec_alu_instance (
                .clk(clk), .reset(reset),
                .enable(thread_en),
                .core_state(core_state),
                .decoded_valu_op(decoded_valu_op),
